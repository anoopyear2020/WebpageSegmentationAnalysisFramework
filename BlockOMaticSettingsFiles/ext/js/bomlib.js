...
function getViXMLObject(obj,level,pid) {
	...
	for (var j=0;j<level;j++) spc+=" ";
        ...
        if (obj.terminal()) {
            ...
            block+=spc+"<Block Ref='Block"+pid+"' ObjectRectText='"+htmlspecialchars(String(obj.dim.t))+"' ID='"+CryptoJS.MD5(internal)+"' internal_id='"+obj.label+"' Pos='WindowWidth||PageRectLeft:" +obj.dim.x+ " WindowHeight||PageRectTop:"+obj.dim.y+" ObjectRectWidth:"+obj.dim.w+" ObjectRectHeight:"+obj.dim.h+"' Doc=''>\n"
            block+=internal;
        } else {
            var tt=spc+"<Block Ref='Block"+pid+"' ObjectRectText='"+htmlspecialchars(String(obj.dim.t))+"' ID='#ID#' Pos='WindowWidth||PageRectLeft:" +obj.dim.x+ " WindowHeight||PageRectTop:"+obj.dim.y+" ObjectRectWidth:"+obj.dim.w+" ObjectRectHeight:"+obj.dim.h+"' Doc=''>\n"
...
function getRect(obj) {
	if (obj.tagName.toUpperCase() == "BODY") {
		return {x:0, y:0, w:documentDim().w, h:documentDim().h, t:obj.outerText}
	} else {
		//~ r=obj.getBoundingClientRect();
		return {x:$(obj).offset().left, y:$(obj).offset().top, w:$(obj).width(), h:$(obj).height(), t:$(obj).text()};
	}
...
function processGeometricObject(element) {
	...
	if (bt) {
		...
		var r = dim.x+" "+dim.y+" "+dim.w+" "+dim.h+" "+dim.t ;
